...
cmd = torch.CmdLine()
cmd:text()
cmd:text('View experiment error matrices at minima')
cmd:text('Example:')
...
local channel = {'optimizer','essrl','expert_error'}
if opt.tester then
   channel = {'tester','essrl','expert_error'}
end
print"spec matrix"
...
if opt.popularity then
   ...
   for i=1,opt.window do
      pop[{{},i}] = opt_sm:max(2):cdiv(opt_sm:sum(2))
